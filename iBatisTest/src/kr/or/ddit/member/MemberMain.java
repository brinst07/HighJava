...
public class MemberMain {
	 ...
	 * 회원 정보를 검색하는 메서드
	 */
	private void getSerchMember() {
		//검색할 회원ID, 회원이름, 전화번호, 주소 등을 입력하면 입력한 정보만 사용하여 검색하는 기능을 구현하시오 주소는 입력한 값이 포함만 되어도 검색 되도록 한다.
		...
		String memAddr = scan.nextLine().trim(); // 공백을 잘라주는 것이 트림
	...
	private void updateMember() {
		boolean chk = false; // 회원이 존재하는지.. 여부 확인..
		String memId;
		...
		if(cnt > 0) {
			System.out.println(memId + "회원의 정보를 성공적으로 수정하였습니다.");
		}else {
			System.out.println(memId + "회원의 정보 수정이 실패하였습니다.");
		}
